...
class StartTile(MapTile): # 상위 클래스: MapTile
    def intro_text(self): # intro_text(self)메소드 오버라이딩
        ...
        어두운 탑 내에서 알라딘을 도와 요술램프를 찾고, 지니를 풀어주세요!
        4개의 방향으로 이동할 수 있으며, 요술램프를 찾아 지니를 도와줄 수 있습니다.
        """
        # 위의 문장 반환
'''
# 지루한 타일 : 아무것도 없음
...
class VictoryTile(MapTile): # 상위 클래스: MapTile
    ...
    def intro_text(self): # 인트로 텍스트 출력
        
            return """
        
            탈출위치에 도착했습니다.
            
            양탄자를 획득하여 지니를 풀어주고 탈출하세요!
    ...
    def modify_player(self, player): #파라미터 1개
        if GameTile.item in player.inventory: #인벤토리에 카펫이 있는지 검사
            print("양탄자가 있습니다..\n요술램프에 소원을 빌어 지니를 풀어줬습니다!\n\n획득한 양탄자를 타고 다음 스테이지로 이동합니다.")
...
class EnemyTile(MapTile): # 상위 클래스: MapTile
    def __init__(self, x, y):   #파라미터 2개
        ...
        if r < 0.50:    
            ...
            self.alive_text = "마법사가 당신을 위협하고 있어요! " \
                              "어서 공격하세요!"
            self.dead_text = "마법사가 사라졌어요!" \
                             "다시 돌아오기 전에 요술램프를 찾으세요!"
            self.run_text = ""
        elif r < 0.80:
            self.enemy = enemies.wizard_subordinate()
            self.alive_text = "마법사의 부하가 나타났어요! 당신을 향해 다가오고 있네요!"
            self.dead_text = "마법사의 부하를 이겼습니다."
            self.run_text = ""
        elif r < 0.95:
            self.enemy = enemies.bandit()
            self.alive_text = "도적이 나타났어요! 어서 물리치세요!"
            self.dead_text = "도적을 물리쳤습니다. 축하합니다"
            self.run_text = ""
        else:
            self.enemy = enemies.guard()
            self.alive_text = "경고. 외부인 침입!" \
                               "경비대에게 걸렸습니다!"
            self.dead_text = "경비대을 물리치고 무사히 도망쳤습니다."
            self.run_text = ""
        '''
...
class GameTile(MapTile):
    ...
    def __init__(self, x, y):
        ...
        self.floor1=trapGame.floor1()   # 다른 모듈에서 GameTile.floor1을 불렀을 경우 trapGame의 floor1클래스(게임) 실행
        
        self.gold = random.randint(1, 50) # 클리어 시 받을 골드 1~50사이 랜덤으로 저장
        #self.item = [items.carpet()]
    ...
    def intro_text(self): # 입장 시 출력 문구
        if self.floor1.is_trap(): #trap함수 반환값이 True일때
            if not self.floor1.is_noclear():
                return """ 함정을 탈출했어요. 계속 나아가세요! """
            else:
                return """ 함정 해결에 실패해 함정이 사라졌습니다. """
        else:
            return """
            앗, 함정에 빠졌어요! 
            문제를 해결해 함정을 통과하세요!
...
class WeaponTile(MapTile):
    ...
    def modify_player(self, player):    # 파라미터 1개
        if not self.weapon_claimed:   # if not self.gold_claimed: 가 True이므로 if문 실행 
            ...
            print("인벤토리에 {}이/가 추가되었습니다.".format(self.weapon)) # "+{}골드가 추가되었습니다." 출력
    ...
    def intro_text(self):
        if self.weapon_claimed: # self.gold_claimed가 True면 아래 문구 출력
            ...
            return"""
            쓸만한 무기가 떨어져 있습니다. 가져가도 좋을 것 같습니다.
            """
