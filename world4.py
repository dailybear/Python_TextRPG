...
class StartTile(MapTile): # 상위 클래스: MapTile
    def intro_text(self): # intro_text(self)메소드 오버라이딩
        ...
        성에는 오래 전 저주를 받아 잠이 든 공주가 있습니다.
        공주를 지키는 경비대원들과 마법사를 피해 공주의 저주를 풀고 공주를 깨워주세요.
        """
        # 위의 문장 반환
...
class VictoryTile(MapTile): # 상위 클래스: MapTile
    ...
    def intro_text(self): # intro_text(self)메소드 오버라이딩
        return """
        탈출위치에 도착했습니다.
        물레를 찾아 파괴하여 공주의 저주를 풀고 탈출에 성공하세요!
    ...
    def modify_player(self, player): #파라미터 1개
        if WeaponTile.item in player.inventory: #인벤토리에 카펫이 있는지 검사
            print(" 물레가 파괴되었어요!\n...\n공주의 저주가 풀리며 공주가 깨어났습니다.\n\n다음 스테이지로 이동합니다.")
...
class EnemyTile(MapTile): # 상위 클래스: MapTile
    def __init__(self, x, y):   #파라미터 2개
        ...
        if r < 0.50:    
            self.enemy = enemies.witch()#마녀
            self.alive_text = "마법사의 부하들이 당신을 위협하고 있습니다." \
                              "어서 물리치세요!"
            self.dead_text = "마법사의 부하들이 사라졌습니다." \
                             "다시 돌아오기 전에 자리를 벗어나세요!"
            self.run_text = ""
        elif r < 0.80:
            self.enemy = enemies.witch_subordinate()#마녀부하
            self.alive_text = "마법사의 조수들이 나타났습니다. "                               
            self.dead_text = "마법사의 조수를 물리치고 무사히 도망쳤습니다."
            self.run_text = ""
        elif r < 0.95:
            self.enemy = enemies.Oldwoman()#노파
            self.alive_text = "공주를 모시는 노파가 당신을 쫓습니다."
            self.dead_text = "노파에게서 무사히 도망쳤습니다."
            self.run_text = ""
        else:
            self.enemy = enemies.guard()
            self.alive_text = "경고. 외부인 침입!" \
                               "경비대에게 걸렸습니다. 경비대를 물리치고 도망치세요!"
            self.dead_text = "경비대를 피해 무사히 도망쳤습니다."
            self.run_text = ""
        '''
...
class WeaponTile(MapTile):
    ...
    def modify_player(self, player):    # 파라미터 1개
        if not self.weapon_claimed:   # if not self.gold_claimed: 가 True이므로 if문 실행 
            ...
            print("방의 구석에 물레가 하나 보입니다. 물레를 얻었습니다.\n")
