...
class StartTile(MapTile): # 상위 클래스: MapTile
    def intro_text(self): # intro_text(self)메소드 오버라이딩
        ...
        어릴 적 마녀에게 납치당한 라푼젤은 이 탑의 꼭대기에 갇혀 있습니다.
        혼자서 탈출하기엔 무리라 생각한 라푼젤은 당신에게 도움을 요청했습니다.
        라푼젤을 도와 탑을 탈출하세요.
        """
        # 위의 문장 반환
...
class VictoryTile(MapTile): # 상위 클래스: MapTile
    ...
    def intro_text(self): # intro_text(self)메소드 오버라이딩
        return """
        최종 탈출 위치에 도착했습니다
        라푼젤의 머리카락을 찾아 머리카락을 타고 탑을 탈출하세요!
    ...
    def modify_player(self, player): #파라미터 1개
        if GameTile.item in player.inventory: #인벤토리에 카펫이 있는지 검사
            print("라푼젤의 잘린 머리카락을 길게 땋아 내려 탑을 내려왔습니다.\n축하합니다.\n탑 탈출에 성공했습니다.")
...
class EnemyTile(MapTile): # 상위 클래스: MapTile
    def __init__(self, x, y):   #파라미터 2개
        ...
        if r < 0.50:    
            ...
            self.alive_text = "마법사가 나타났습니다. " \
                              "어서 공격하세요!"
            self.dead_text = "마법사를 물리쳤습니다." 
                            
            self.run_text = ""
        elif r < 0.80:
            self.enemy = enemies.wizard_subordinate()
            self.alive_text = "마녀의 부하가 나타났습니다!"                               
            self.dead_text = "마녀의 부하를 물리쳤습니다."
            self.run_text = ""
        elif r < 0.95:
            self.enemy = enemies.bandit()
            self.alive_text = "마녀가 고용한 도적이 나타났습니다."
            self.dead_text = "도적을 물리쳤습니다!"
            self.run_text = ""
        else:
            self.enemy = enemies.guard()
            self.alive_text = "경고. 외부인 침입!" \
                               "탑을 지키는 경비원에게 들켰습니다. 경비원을 물리치세요!"
            self.dead_text = "경비원을 물리쳤습니다."
            self.run_text = ""
        '''
...
class GameTile(MapTile):
    ...
    def __init__(self, x, y):
        ...
        self.floor5 = trapGame5.floor5()   # 다른 모듈에서 GameTile.floor1을 불렀을 경우 trapGame의 floor1클래스(게임) 실행
        
        self.gold = random.randint(1, 50) # 클리어 시 받을 골드 1~50사이 랜덤으로 저장
        #self.item = [items.Hair()]
    ...
    def intro_text(self): # 입장 시 출력 문구
        if self.floor5.is_trap(): #trap함수 반환값이 True일때
            if not self.floor5.is_noclear():
    ...
    def modify_player(self, player):
        if not self.floor5.is_noclear(): #문제를 못 풀지 않으면
            if not self.gold_claimed:
                player.gold = player.gold + self.gold # 플레이어의 골드에 랜덤으로 받은 골드 추가
                print("+{}골드가 추가되었습니다.".format(self.gold))
                player.inventory.append(self.item)
                print("마녀가 라푼젤을 데리고 도망친 자리엔 라푼젤의 잘린 머리카락만이 남아있습니다.\n")
                print("라푼젤의 머리카락을 얻었습니다.")
